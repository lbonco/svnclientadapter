    		...
    		if (!available) {
    			...
    			try {
    				...
    				if (version.getMajor() == 1 && version.getMinor() == 13)
    					...
    					available = false;
    					javaHLErrors = new StringBuffer("Incompatible JavaHL library loaded.  Subversion 1.13.x required.");
    				}
    			} catch (UnsatisfiedLinkError e) {
    				available = false;
    				javaHLErrors = new StringBuffer("Incompatible JavaHL library loaded.  1.13.x or later required.");
    			}
    		}
    	}
